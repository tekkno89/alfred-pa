...
def validate_config(settings: Settings) -> list[ConfigIssue]:
    ...
    if settings.coding_runtime_provider == "docker_sandbox":
        if not settings.sandbox_url:
            issues.append(ConfigIssue(
                "coding_assistant", "error", "SANDBOX_URL",
                "Required for docker_sandbox runtime",
            ))
        if not settings.sandbox_api_key:
            issues.append(ConfigIssue(
                "coding_assistant", "warning", "SANDBOX_API_KEY",
    ...
    if settings.claude_code_provider == "api":
        if not settings.claude_code_api_key:
            issues.append(ConfigIssue(
                "coding_assistant", "error", "CLAUDE_CODE_API_KEY",
        ...
        if not settings.claude_code_vertex_project:
            issues.append(ConfigIssue(
                "coding_assistant", "error", "CLAUDE_CODE_VERTEX_PROJECT",
                "Vertex provider selected but no project configured",
            ))
        has_creds = any([
            settings.claude_code_gcp_sa_json,
            settings.claude_code_gcp_credentials_file,
            settings.claude_code_gcp_adc_path,
        ])
        if not has_creds:
            issues.append(ConfigIssue(
                "coding_assistant", "warning", "CLAUDE_CODE_GCP_*",
                "Vertex provider selected but no GCP credentials configured for containers",
            ))

    # Completion method
    if settings.coding_completion_method == "callback" and not settings.coding_callback_base_url:
        issues.append(ConfigIssue(
            "coding_assistant", "warning", "CODING_CALLBACK_BASE_URL",
            "Callback completion method requires a base URL — containers won't be able to report completion",
        ))

    # Event bus
    if settings.coding_event_bus_provider == "gcp_pubsub" and not settings.coding_gcp_pubsub_project:
        issues.append(ConfigIssue(
            "coding_assistant", "error", "CODING_GCP_PUBSUB_PROJECT",
    ...
    if settings.slack_bot_token and not settings.slack_signing_secret:
        issues.append(ConfigIssue(
            "slack", "error", "SLACK_SIGNING_SECRET",
    ...
    if settings.github_app_id and not settings.github_client_id:
        issues.append(ConfigIssue(
            "github", "warning", "GITHUB_CLIENT_ID",
...
def get_service_statuses(settings: Settings) -> list[ServiceStatus]:
    ...
    services = [
        ServiceStatus(
            ...
            enabled=bool(settings.sandbox_url or settings.coding_runtime_provider != "docker_sandbox"),
            details={
